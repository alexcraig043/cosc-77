...
Mesh loadWavefrontOBJ(const std::string & filename, const Transform & xform)
{
    ...
    while (std::getline(is, line_str))
    {
        ...
        else if (prefix == "vn")
        {
            Normal3f n;
            line >> n.x() >> n.y() >> n.z();
            normals.push_back((xform * n).normalized());
        }
        else if (prefix == "f")
        {
    ...
    for (auto i : range(int(mesh.F.size())))
        mesh.F[i] = Vector3i(indices[3*i], indices[3*i+1], indices[3*i+2]);
    ...
    message("xform:\n %s\n", xform.getMatrix());
    message("bounding box: min:\n%s;\nmax:\n%s.\n", bbox.min(), bbox.max());

    message("bounding box: bottom:\n%s;\n", (bbox.min()+bbox.max())/2.f - Vector3f(0, bbox.diagonal()[1]/2.f, 0));

    message("done. (V=%d, F=%d, took %s and %s)\n",
            mesh.V.size(), mesh.F.size(), timer.elapsedString().c_str(),
            memString(mesh.F.size() * sizeof(uint32_t) + sizeof(float) *
                      (mesh.V.size() + mesh.N.size() + mesh.UV.size())).c_str());
